...
const SubscribeStrategySuggestDetail : React.FC<Props> = ({strategys, apis, getData=()=>{}, handdleHideModal=()=>{}}) => {
  ...
  const validateForm = () => {
    ...
    ///let minCapital = 0//Number(strategyObj!.data!.min_capital)
  ...
  const handleSubscribe = async () => {
    ...
    setSaveDataFlag(1)
    strategys.map( async(strategyObj) => {
              ...
              .then( async()=>{
                await setDoc(doc(db, `users/${userId}/accounts/${_apiKey}/strategys`, strategyObj!.id ), 
                  ...
                  .then( async()=>{
                    ...
                    //statement
                    await setDoc(doc(db,  `users/${userId}/accounts/${_apiKey}/strategys/${strategyObj!.id}/statement`, `${userId}_${strategyObj!.id}_statement`), 
                    {  
                      cd: Timestamp.now() }
                    ).then( async()=>{
                      //create file in storage
                    ...
                    //lending hist 1
                    await setDoc(doc(db,  `users/${userId}/accounts/${_apiKey}/strategys/${strategyObj!.id}/lending_hist_01`, `${userId}_${strategyObj!.id}_lending_hist_01`), 
                    {  
                      cd: Timestamp.now()}
                    ).then( async()=>{
                      //create file in storage
                    ...
                    //lending hist 2
                    await setDoc(doc(db,  `users/${userId}/accounts/${_apiKey}/strategys/${strategyObj!.id}/lending_hist_02`, `${userId}_${strategyObj!.id}_lending_hist_02`), 
                    {  
                      cd: Timestamp.now()}
                    ).then( async()=>{
                      //create file in storage
                    ...
                    //rp
                    await setDoc(doc(db,  `users/${userId}/accounts/${_apiKey}/strategys/${strategyObj!.id}/rp`, `${userId}_${strategyObj!.id}_rp`), 
                    {  NO:''
                        ,Zone:''
                        ,TP_Zone:''
                        ,Size_USD:''
...
export {SubscribeStrategySuggestDetail}